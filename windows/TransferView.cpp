...
LRESULT TransferView::onCreate(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/) {
	...
	ResourceLoader::LoadImageList(IDR_ARROWS, arrows, 16, 16);
	ResourceLoader::LoadImageList(IDR_TSPEEDS, speedImages, 16, 16);
	ResourceLoader::LoadImageList(IDR_TSPEEDSBW, speedImagesBW, 16, 16);
	...
	if (reinterpret_cast<HWND>(wParam) == ctrlTransfers && ctrlTransfers.GetSelectedCount() > 0) { 
		...
		if(ii->download) {
			if(!ii->target.empty()) {
				...
				QueueManager::getInstance()->lockedOperation([&slowDisconnect, &target](const QueueItem::StringMap& queue) {
...
void TransferView::ItemInfo::removeAll() {
	if(hits <= 1) {
		...
		if(checkState == BST_CHECKED || ::MessageBox(0, _T("Do you really want to remove this item?"), _T(APPNAME) _T(" ") _T(VERSIONSTRING), CTSTRING(DONT_ASK_AGAIN), MB_YESNO | MB_ICONQUESTION | MB_DEFBUTTON2, checkState) == IDYES)
			QueueManager::getInstance()->remove(Text::fromT(target));
		// Let's update the setting unchecked box means we bug user again...
		SettingsManager::getInstance()->set(SettingsManager::CONFIRM_DELETE, checkState != BST_CHECKED);
	}
}
...
LRESULT TransferView::onCustomDraw(int /*idCtrl*/, LPNMHDR pnmh, BOOL& bHandled) {
	...
	case CDDS_SUBITEM | CDDS_ITEMPREPAINT: {
		...
		if((colIndex == COLUMN_STATUS) && (ii->status != ItemInfo::STATUS_WAITING)) {
			...
			COLORREF oldcol;
			if(BOOLSETTING(PROGRESSBAR_ODC_STYLE) || BOOLSETTING(STEALTHY_STYLE)) {
					...
					if(indicateSpeeds) {
						...
						if(ii->type == Transfer::TYPE_FULL_LIST || ii->type == Transfer::TYPE_PARTIAL_LIST) {
							...
							if(!BOOLSETTING(THROTTLE_ENABLE)) {
								...
								if(!ii->download) {
									speedmark = ThrottleManager::getInstance()->getUpLimit() / 5;
								} else {
									speedmark = ThrottleManager::getInstance()->getDownLimit() / 5;
								}
							}
...
void TransferView::on(ConnectionManagerListener::StatusChanged, const ConnectionQueueItem* aCqi) noexcept {
	UpdateInfo* ui = new UpdateInfo(aCqi->getUser(), aCqi->getDownload());
...
const tstring TransferView::ItemInfo::getText(uint8_t col) const {
	switch(col) {
		case COLUMN_USER: return (hits == -1) ? WinUtil::getNicks(user) : (Util::toStringT(hits) + _T(' ') + TSTRING(USERS));
		case COLUMN_HUB: return (hits == -1) ? WinUtil::getHubNames(user).first : (Util::toStringT(running) + _T(' ') + TSTRING(NUMBER_OF_SEGMENTS));
		...
		case COLUMN_SPEED: return (status == STATUS_RUNNING) ? (Util::formatBytesT(speed) + _T("/s")) : Util::emptyStringT;
		case COLUMN_FILE: return getFile(type, Util::getFileName(target));
		case COLUMN_SIZE: return Util::formatBytesT(size); 
		case COLUMN_PATH: return Util::getFilePath(target);
		case COLUMN_IP: return ip;
		case COLUMN_RATIO: return (status == STATUS_RUNNING) ? Util::toStringT(getRatio()) : Util::emptyStringT;
...
void TransferView::on(DownloadManagerListener::Requesting, const Download* d) noexcept {
	...
	if(d->getType() != Transfer::TYPE_FULL_LIST && d->getType() != Transfer::TYPE_PARTIAL_LIST) {
		...
		ui->setStatusString(TSTRING(REQUESTING_LIST) + _T("..."));
	}
...
void TransferView::on(DownloadManagerListener::Tick, const DownloadList& dl) noexcept {
	for(DownloadList::const_iterator j = dl.begin(); j != dl.end(); ++j) {
		...
		tstring pos = Util::formatBytesT(d->getPos());
		...
		statusString += TSTRING_F(DOWNLOADED_BYTES, pos.c_str() % percent % elapsed.c_str());
...
void TransferView::on(UploadManagerListener::Starting, const Upload* aUpload) noexcept {
	...
	ui->setStatus(ItemInfo::STATUS_RUNNING);
	ui->setActual(aUpload->getActual(true));
	ui->setSize(aUpload->getSize(true));
	ui->setRunning(1);
...
void TransferView::on(UploadManagerListener::Tick, const UploadList& ul) noexcept {
	for(UploadList::const_iterator j = ul.begin(); j != ul.end(); ++j) {
		...
		ui->setActual(u->getActual(true));
		ui->setPos(u->getPos(true));
		ui->setTimeLeft(u->getSecondsLeft(true));
		ui->setSpeed(static_cast<int64_t>(u->getAverageSpeed()));

		tstring pos = Util::formatBytesT(ui->pos);
		double percent = (double)u->getPos(true) * 100.0 / (double)u->getSize(true);
		...
		statusString += TSTRING_F(UPLOADED_BYTES, pos.c_str() % percent % elapsed.c_str());
		ui->setStatusString(statusString);
...
LRESULT TransferView::onPreviewCommand(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/){
	...
	while((i = ctrlTransfers.GetNextItem(i, LVNI_SELECTED)) != -1) {
		...
		QueueManager::getInstance()->lockedOperation([&target, &tmpTarget](const QueueItem::StringMap& queue) {
			auto qi = queue.find(&target);
			if(qi != queue.end())
				tmpTarget = qi->second->getTempTarget();
		});

		WinUtil::RunPreviewCommand(wID - IDC_PREVIEW_APP, tmpTarget);
	}
...
LRESULT TransferView::onSlowDisconnect(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/) {
	...
	while((i = ctrlTransfers.GetNextItem(i, LVNI_SELECTED)) != -1) {
		...
		QueueManager::getInstance()->lockedOperation([&target](const QueueItem::StringMap& queue) {
			auto qi = queue.find(&target);
		...
		if(segs > 0) {
			...
			if(qi->getFileBegin() == 0) {
				...
				if(BOOLSETTING(POPUP_DOWNLOAD_START))
					WinUtil::showPopup(TSTRING(FILE) + _T(": ") + Util::getFileName(ui->target), TSTRING(DOWNLOAD_STARTING));
			} else {
...
void TransferView::on(QueueManagerListener::Finished, const QueueItem* qi, const string&, const Download* download) noexcept {
	...
	if(BOOLSETTING(POPUP_DOWNLOAD_FINISHED))
		WinUtil::showPopup(TSTRING(FILE) + _T(": ") + Util::getFileName(ui->target), TSTRING(DOWNLOAD_FINISHED_IDLE));
