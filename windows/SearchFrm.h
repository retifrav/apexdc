...
#define SEARCHBOX_MESSAGE_MAP 9
...
public:
	...
	ALT_MSG_MAP(SEARCHBOX_MESSAGE_MAP)
		MESSAGE_HANDLER(WM_CHAR, onCharSearch)
		MESSAGE_HANDLER(WM_KEYDOWN, onCharSearch)
		MESSAGE_HANDLER(WM_KEYUP, onCharSearch)
	...
	SearchFrame() : 
		searchBoxContainer(WC_COMBOBOX, this, SEARCHBOX_MESSAGE_MAP),
		searchContainer(WC_EDIT, this, SEARCHBOX_MESSAGE_MAP), 
	...
	LRESULT onCharSearch(UINT uMsg, WPARAM wParam, LPARAM /*lParam*/, BOOL& bHandled);
	...
	public:
		...
		const tstring getText(uint8_t col) const {
			switch(col) {
				...
				case COLUMN_HITS: return hits == 0 ? Util::emptyStringT : Util::toStringT(hits + 1) + _T(' ') + TSTRING(USERS);
				case COLUMN_NICK: return WinUtil::getNicks(sr->getUser().user, sr->getUser().hint);
				case COLUMN_TYPE:
					if(sr->getType() == SearchResult::TYPE_FILE) {
				...
				case COLUMN_EXACT_SIZE: return sr->getSize() > 0 ? Text::toT(Util::formatExactSize(sr->getSize())) : Util::emptyStringT;
				case COLUMN_IP: {
					string ip = sr->getIP();
					if (!ip.empty()) {
						// Only attempt to grab a country mapping if we actually have an IP address
			...
			switch(col) {
				case COLUMN_TYPE: 
					if(a->sr->getType() == b->sr->getType())
						return Util::DefaultSort(a->getText(COLUMN_TYPE).c_str(), b->getText(COLUMN_TYPE).c_str());
					else
				...
				case COLUMN_CONNECTION: {
					int64_t a_numericConn, b_numericConn;
					tstring a_str = Text::toT(ClientManager::getInstance()->getConnection(a->getUser()->getCID(), a_numericConn)),
						b_str = Text::toT(ClientManager::getInstance()->getConnection(b->getUser()->getCID(), b_numericConn));
					...
					if(a_numericConn == -1 && b_numericConn != -1)
						return -1;
					return Util::DefaultSort(a_str.c_str(), b_str.c_str());
				}
				case COLUMN_SIZE:
		...
		int getImageIndex() const {
			...
			if (BOOLSETTING(USE_SYSTEM_ICONS)) {
				...
				string tmp = ClientManager::getInstance()->getConnection(sr->getUser().user->getCID());
				if( (tmp == "28.8Kbps") ||
					(tmp == "33.6Kbps") ||
					(tmp == "56Kbps") ||
					(tmp == "Modem") ||
