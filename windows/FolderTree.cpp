...
ShareEnumerator::ShareEnumerator()
{
	...
	m_bWinNT = (WinUtil::getVersionInfo(osvi) && osvi.dwPlatformId == VER_PLATFORM_WIN32_NT);
	if (m_bWinNT)
	{
	...
	if (m_bWinNT)
	{
		if (m_pNTShareInfo)
		{
			for (DWORD i=0; i<m_dwShares && !bShared; i++)
			{
				...
				bShared = (stricmp(sPath.c_str(), sShare.c_str()) == 0) && ((m_pNTShareInfo[i].shi502_type == STYPE_DISKTREE) || ((m_pNTShareInfo[i].shi502_type == STYPE_PRINTQ)));
			}
		}
	}
	else
	{
		if (m_pWin9xShareInfo)
		{
			for (DWORD i=0; i<m_dwShares && !bShared; i++)
			{
				...
				bShared = (stricmp(sPath.c_str(), sShare.c_str()) == 0) &&
...
void FolderTree::DoExpand(HTREEITEM hItem)
{
	...
	if (IsMediaValid(pItem->m_sFQPath))
	{
		...
		else if ((hItem == m_hNetworkRoot) || (pItem->m_pNetResource))
		{
			...
			CCursor wait;
			wait.LoadSysCursor(IDC_WAIT);
			HCURSOR oldCursor = SetCursor(wait);
...
HTREEITEM FolderTree::FindSibling(HTREEITEM hParent, const tstring &sItem) const
{
	...
	while(hChild)
	{
		...
		if (stricmp(pItem->m_sRelativePath.c_str(), sItem.c_str()) == 0)
			return hChild;
...
void FolderTree::UpdateStaticCtrl()
{
	...
	if(m_pStaticCtrl != NULL)
	{
		...
		m_pStaticCtrl->SetWindowText((Util::formatBytesT(ShareManager::getInstance()->getShareSize() + shareSize) + _T("*")).c_str());
	}
}
...
bool FolderTree::IsPathValid(const tstring &sPath) {
	if(sPath.empty())
		return false;
