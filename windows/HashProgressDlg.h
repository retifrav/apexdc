...
public:
	...
	LRESULT OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/) {
		...
		SetDlgItemText(IDC_MAX_HASH_SPEED, Util::toStringT(SETTING(MAX_HASH_SPEED)).c_str());
	...
	void updateStats() {
		...
		if(diff < 1000 || files == 0 || bytes == 0 || paused) { // KUL - hash progress dialog patch
			...
			SetDlgItemText(IDC_HASH_SPEED, (_T("-.-- B/s, ") + Util::formatBytesT(bytes) + _T(" ") + TSTRING(LEFT)).c_str());
			// KUL - hash progress dialog patch
			if(paused) {
			...
			SetDlgItemText(IDC_HASH_SPEED, (Util::formatBytesT((int64_t)speedStat) + _T("/s, ") + Util::formatBytesT(bytes) + _T(" ") + TSTRING(LEFT)).c_str());
