...
LRESULT ADLSearchFrame::onItemChanged(int /*idCtrl*/, LPNMHDR pnmh, BOOL& /*bHandled*/) 
{
	...
	::EnableWindow(GetDlgItem(IDC_EDIT), (lv->uNewState & LVIS_FOCUSED) || ((lv->uNewState & LVIS_STATEIMAGEMASK) && ctrlList.GetNextItem(-1, LVNI_SELECTED) != -1));
	::EnableWindow(GetDlgItem(IDC_REMOVE), (lv->uNewState & LVIS_FOCUSED) || ((lv->uNewState & LVIS_STATEIMAGEMASK) && ctrlList.GetNextItem(-1, LVNI_SELECTED) != -1));
...
void ADLSearchFrame::UpdateSearch(int index, BOOL doDelete)
{
	...
	fs = _T("");
	if(search.adlsPriority >= 0)
	{
		fs = Util::toStringT(search.adlsPriority);
	}
	line.push_back(fs);
	line.push_back(search.isForbidden ? RawManager::getInstance()->getNameActionId(RawManager::getInstance()->getValidAction(search.raw)) : (search.isAutoQueue ? TSTRING(ADLS_DOWNLOAD) : TSTRING(NO_ACTION)));
	line.push_back(Text::toT(search.adlsComment + search.getConfStr()));
