...
void OperaColors::StealthyFill(CDC& dc, int x1, int y1, int x2, int y2, COLORREF c) {
	...
	// To avoid distinctive border colours, think about this...
	COLORREF penClr = ((HLS_S(::RGB2HLS(c)) > 30) ? HLS_TRANSFORM(c, -40, 50) : HLS_TRANSFORM(c, -15, 0));

	HGDIOBJ oldPen = ::SelectObject(dc, CreatePen(PS_SOLID, 1, penClr));
	HGDIOBJ oldBrush = ::SelectObject(dc, CreateSolidBrush(c));
	...
	// Draw bar highlight
	if((x2 - x1) > 4) {
		oldPen = ::SelectObject(dc, CreatePen(PS_SOLID, 1, HLS_TRANSFORM(c, 20, -30)));
