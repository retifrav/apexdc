...
LRESULT SearchFrame::onCreate(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& bHandled)
{
	...
	lastSearches = SearchManager::getInstance()->getSearchHistory();
	...
	if (BOOLSETTING(USE_SYSTEM_ICONS)) {
		...
		ResourceLoader::LoadImageList(IDR_SPEEDS, images, 16, 16);
...
LRESULT SearchFrame::onDrawItem(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
	...
	if(wParam == IDC_FILETYPES) {
		...
		if(searchStartTime > 0){
			...
			OperaColors::StealthyFill(dc, rc.left, rc.top,  rc.left + (LONG)length, rc.bottom, RGB(128,128,128));

			dc.SetBkMode(TRANSPARENT);

			uint64_t percent = (now - searchStartTime) * 100 / (searchEndTime - searchStartTime);
			tstring progress = percent >= 100 ? TSTRING(DONE) : Util::toStringT(percent) + _T("%");
...
void SearchFrame::onEnter() {
	...
	
	{
		...
		if(find(lastSearches.begin(), lastSearches.end(), s) == lastSearches.end()) 
		{
			...
			int pos = ctrlSearchBox.FindString(-1, s.c_str());
...
void SearchFrame::SearchInfo::CheckTTH::operator()(SearchInfo* si) {
	...
	if(firstHubs && hubs.empty()) {
		hubs = ClientManager::getInstance()->getHubs(si->sr->getUser());
		firstHubs = false;
	} else if(!hubs.empty()) {
		// we will merge hubs of all users to ensure we can use OP commands in all hubs
		StringList sl = ClientManager::getInstance()->getHubs(si->sr->getUser());
...
LRESULT SearchFrame::onDownloadFavoriteDirs(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/) {
	...
	FavoriteManager::FavDirList spl = FavoriteManager::getInstance()->getFavoriteDirs();
	if(newId < spl.size()) {
		ctrlResults.forEachSelectedT(SearchInfo::Download(Text::toT(spl[newId].dir), this));
	} else {
...
LRESULT SearchFrame::onDownloadWholeFavoriteDirs(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/) {
	...
	ctrlResults.forEachSelectedT(SearchInfo::DownloadWhole(Text::toT(spl[wID-IDC_DOWNLOAD_WHOLE_FAVORITE_DIRS].dir)));
	return 0;
}
...
LRESULT SearchFrame::onDoubleClickResults(int /*idCtrl*/, LPNMHDR pnmh, BOOL& /*bHandled*/) {
	...
	if (item->iItem != -1) {
		...
		while((i = ctrlResults.GetNextItem(i, LVNI_SELECTED)) != -1) {
			...
			string t = FavoriteManager::getInstance()->getDownloadDirectory(Util::getFileExt(si->sr->getFileName()));		
			(SearchInfo::Download(Text::toT(t), this))(si);
		}
	}
	return 0;
}
...
LRESULT SearchFrame::onClose(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& bHandled)
{
	if(!closed) {
		...
		SearchManager::getInstance()->setSearchHistory(lastSearches);
...
void SearchFrame::runUserCommand(UserCommand& uc) {
	...
	while((sel = ctrlResults.GetNextItem(sel, LVNI_SELECTED)) != -1) {
		...
		if(sr->getType() == SearchResult::TYPE_FILE) {
			...
			ucParams["fileMN"] = WinUtil::getMagnet(sr->getTTH(), sr->getFile(), sr->getSize());
		}
...
LRESULT SearchFrame::onCharSearch(UINT uMsg, WPARAM wParam, LPARAM /*lParam*/, BOOL& bHandled) {
	if(uMsg == WM_CHAR)
		if(wParam == VK_BACK)
...
void SearchFrame::addSearchResult(SearchInfo* si) {
	...
	if(!si->getText(COLUMN_TTH).empty() && useGrouping) {
		...
		if(pp) {
			if((sr->getUser().user->getCID() == pp->parent->getUser()->getCID()) && (sr->getFile() == pp->parent->sr->getFile())) {	 	
				delete si;
				return;	 	
			} 	
			for(vector<SearchInfo*>::const_iterator k = pp->children.begin(); k != pp->children.end(); k++){	 	
				if((sr->getUser().user->getCID() == (*k)->getUser()->getCID()) && (sr->getFile() == (*k)->sr->getFile())) {	 	
					delete si;
					return;	 	
				} 	
			}	 	
		...
		for(SearchInfoList::ParentMap::const_iterator s = ctrlResults.getParents().begin(); s != ctrlResults.getParents().end(); ++s) {
			...
			if((sr->getUser().user->getCID() == sr2->getUser().user->getCID()) && (sr->getFile() == sr2->getFile())) {
				delete si;	 	
				return;	 	
			}
		}	 	
	...
	if(running) {
		...
		ctrlStatus.SetText(3, (Util::toStringT(resultsCount) + _T("/") + Util::toStringT(pausedResults.size() + resultsCount) + _T(" ") + WSTRING(FILES)).c_str());
	}
}
...
LRESULT SearchFrame::onContextMenu(UINT /*uMsg*/, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
	if (reinterpret_cast<HWND>(wParam) == ctrlResults && ctrlResults.GetSelectedCount() > 0) {
		...
		if(ctrlResults.GetSelectedCount() > 0) {
			...
			if(ctrlResults.GetSelectedCount() > 1)
				resultsMenu.InsertSeparatorFirst(Util::toStringT(ctrlResults.GetSelectedCount()) + _T(" ") + TSTRING(FILES));
			else
				resultsMenu.InsertSeparatorFirst(Util::toStringT(((SearchInfo*)ctrlResults.getSelectedItem())->hits + 1) + _T(" ") + TSTRING(USERS));
			...
			FavoriteManager::FavDirList spl = FavoriteManager::getInstance()->getFavoriteDirs();
			if (spl.size() > 0) {
				for(FavoriteManager::FavDirIter i = spl.begin(); i != spl.end(); i++) {
					targetMenu.AppendMenu(MF_STRING, IDC_DOWNLOAD_FAVORITE_DIRS + n, Text::toT(i->name).c_str());
					n++;
				}
			...
			if (spl.size() > 0) {
				for(FavoriteManager::FavDirIter i = spl.begin(); i != spl.end(); ++i) {
					targetDirMenu.AppendMenu(MF_STRING, IDC_DOWNLOAD_WHOLE_FAVORITE_DIRS + n, Text::toT(i->name).c_str());
					n++;
				}
...
void SearchFrame::initHubs() {
	...
	auto endIt = clients.cend();
	for(auto it = clients.cbegin(); it != endIt; ++it) {
...
LRESULT SearchFrame::onPause(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/) {
	if(!running) {
		...
		running = false;
		ctrlPauseSearch.SetWindowText(CTSTRING(CONTINUE));
	}
	return 0;
}
...
LRESULT SearchFrame::onCopy(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/) {
	...
	if ( pos >= 0 ) {
		...
		switch (wID) {
			case IDC_COPY_NICK:
				sCopy = WinUtil::getNicks(sr->getUser().user, sr->getUser().hint);
				break;
			...
			case IDC_COPY_PATH:
				...
				break;
			case IDC_COPY_SIZE:
				sCopy = Util::formatBytesT(sr->getSize());
				break;
			case IDC_COPY_LINK:
				if(sr->getType() == SearchResult::TYPE_FILE) {
...
LRESULT SearchFrame::onCustomDraw(int /*idCtrl*/, LPNMHDR pnmh, BOOL& /*bHandled*/) {
	...
	case CDDS_ITEMPREPAINT: {
		...
		if(si->sr != NULL) {
			...
			} else if(ShareManager::getInstance()->isTTHShared(si->sr->getTTH())) {
				cd->clrText = SETTING(COLOUR_DUPE);
			} if(!si->sr->getUser().user->isOnline() && BOOLSETTING(BLEND_OFFLINE_SEARCH)) {
				cd->clrText = OperaColors::blendColors(cd->clrTextBk, cd->clrText, 0.4);
			}
		}
...
LRESULT SearchFrame::onEditChange(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& bHandled) {
	if(BOOLSETTING(SEARCH_DETECT_TTH)) {
		size_t sz = ctrlSearchBox.GetWindowTextLength();
		if (sz == 39) {
			tstring buf(40, _T('\0'));
			ctrlSearchBox.GetWindowText(&buf[0], buf.size());
			buf.resize(buf.size()-1);
			try {
				boost::wregex reg(_T("^[A-Z2-7]{38}[QYAI]$"), boost::regex_constants::icase);
				if(boost::regex_match(buf, reg)) {
					ctrlFiletype.SetCurSel(8);
					return 0;
				}
			} catch(...) { }
		}
	...
	HWND hWndCombo = ctrlSearchBox.m_hWnd;
