...
void PopupManager::Show(const tstring &aMsg, const tstring &aTitle, int Icon, HICON hIcon /*= NULL*/, bool force /*= false*/) {
	if(!force) {
		if(!Util::getAway() && BOOLSETTING(POPUP_AWAY))
			return;
	
		if(!MainFrame::getMainFrame()->getAppMinimized() && BOOLSETTING(POPUP_MINIMIZED))
			return;
	}
	...
	if(SETTING(POPUP_TYPE) == BALLOON) {
		...
		if((m_nid.dwInfoFlags & NIIF_USER) == NIIF_USER) {
	...
	if(PopupImage != SETTING(POPUPFILE) || popuptype != SETTING(POPUP_TYPE)) {
		...
		hBitmap.Load(Text::toT(PopupImage).c_str());
