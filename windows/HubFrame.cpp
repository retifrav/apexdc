...
LRESULT HubFrame::OnCreate(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& bHandled)
{
	...
	ResourceLoader::LoadImageList(IDR_STATUSBAR, statusImgList, 16, 16);

	for(int idx=0;idx<5;++idx)	
		hndStatusIcons[idx]=statusImgList.GetIcon(idx, ILD_IMAGE);
	...
	SetSplitterPosPct((hubchatusersplit && !BOOLSETTING(GLOBAL_HUBFRAME_CONF) ? hubchatusersplit : 7500) / 100);
	...
	if(fhe && !BOOLSETTING(GLOBAL_HUBFRAME_CONF)) {
	...
	if(fhe) {
		createHubTab(BOOLSETTING(GLOBAL_MINI_TABS) ? !fhe->getMiniTab() : fhe->getMiniTab());
	} else {
		createHubTab(BOOLSETTING(GLOBAL_MINI_TABS));
	}
...
bool HubFrame::openWindow(const tstring& aServer, int chatusersplit, bool userliststate) {
#if BETA
	if(!UpdateManager::getInstance()->isLoggedIn())
		return false;
#endif

	const string& blacklisted = FavoriteManager::getInstance()->blacklisted(Text::fromT(aServer));
	if(!blacklisted.empty()) {
			auto choice = ::MessageBox(WinUtil::mainWnd, (dcpp_fmt(_T("Warning: potentially fradulent hub connection detected!\r\n\r\n")
			_T("The hub with a following address %1% is using a domain that has been blacklisted by ApexDC++ for the following reason:\r\n")
			_T("%2%\r\n\r\n")
			_T("It is strongly recommended that you do not connect to this hub, or do so knowing the hub may not be the one you are expecting, ")
			_T("would you like to continue regardless?")
			) % aServer % Text::toT(blacklisted)).str().c_str(), _T(APPNAME) _T(" ") _T(VERSIONSTRING), MB_ICONINFORMATION | MB_YESNO);

			if(choice == IDNO) {
				auto fav = FavoriteManager::getInstance()->getFavoriteHubEntry(Text::fromT(aServer));
				if(fav) {
					if(::MessageBox(WinUtil::mainWnd, _T("Would you like to remove the hub using this domain from your favorite hubs list? \r\n\r\n"), _T(APPNAME) _T(" ") _T(VERSIONSTRING), MB_ICONQUESTION | MB_YESNO) == IDYES)
						FavoriteManager::getInstance()->removeFavorite(fav);
				}
				return false;
			}
	}
	...
	if(ctrlMessage.GetWindowTextLength() > 0) {
		...
		if(s[0] == _T('/')) {
			...
			if(PluginManager::getInstance()->onChatCommand(client, Text::fromT(s))) {
				...
				if(param.empty()) {
					WinUtil::openFile(Text::toT(Util::validateFileName(LogManager::getInstance()->getPath(LogManager::CHAT, params))));
				} else if(stricmp(param.c_str(), _T("status")) == 0) {
					WinUtil::openFile(Text::toT(Util::validateFileName(LogManager::getInstance()->getPath(LogManager::STATUS, params))));
				}
				...
				auto ignoredUsers = FavoriteManager::getInstance()->getIgnoredUsers();
				for(auto i = ignoredUsers.cbegin(); i != ignoredUsers.cend(); ++i)
			...
			} else if(stricmp(cmd.c_str(), _T("help")) == 0 || stricmp(cmd.c_str(), _T("h")) == 0) {
				addLine(_T("*** ") + WinUtil::commands + _T(""), WinUtil::m_ChatTextSystem);
			...
			} else if(stricmp(cmd.c_str(), _T("pubstats")) == 0) {
				client->hubMessage(WinUtil::generateStats());
			} else if(stricmp(cmd.c_str(), _T("reconnect")) == 0 || stricmp(cmd.c_str(), _T("rc")) == 0) {
				PostMessage(WM_COMMAND, ID_FILE_RECONNECT, 0);
			} else if(stricmp(cmd.c_str(), _T("sc")) == 0 || stricmp(cmd.c_str(), _T("startchecking")) == 0) {
				if(!client->isOp()) {
					addStatus(TSTRING(OP_ONLY), WinUtil::m_ChatTextSystem);
				} else if(client->startChecking()) {
					addStatus(TSTRING(CHECKING_STARTED), WinUtil::m_ChatTextSystem);
				} else {
					client->stopChecking();
					addStatus(TSTRING(CHECKING_STOPPED), WinUtil::m_ChatTextSystem);
				}
			} else {
		...
		switch (wID) {
			...
			case IDC_COPY_HUBADDRESS:
				...
				if(client->isSecure() && client->getHubUrl().find("kp=") == string::npos) {
					sCopy += sCopy[sCopy.size() - 1] != '/' ? "/?kp=" : "?kp=";
					sCopy += client->getKeyprintString();
				}
				break;
		}
...
LRESULT HubFrame::onSpeaker(UINT /*uMsg*/, WPARAM /* wParam */, LPARAM /* lParam */, BOOL& /*bHandled*/) {
	...
	for(auto i = t.begin(); i != t.end(); ++i) {
		if(i->first == UPDATE_USER) {
			...
			if(client->getAutoOpenOpChat() && !opened) {
				if(u.onlineUser->getIdentity().getNick() == client->getOpChat() && !PrivateFrame::isOpen(u.onlineUser->getUser())) {
					PrivateFrame::openWindow(HintedUser(u.onlineUser->getUser(), client->getHubUrl()), Util::emptyStringT, client);
					opened = true; 
				}
			}
			...
			ctrlStatus.SetIcon(1, client->isSecure()?hndStatusIcons[StatusIcons::SECURE_HUB]:hndStatusIcons[StatusIcons::NOT_SECURE_HUB]);
			
			statusSizes[0] = WinUtil::getTextWidth(text, ctrlStatus.m_hWnd) + 23;
			...
			if(!fromUrl.empty()) {
				auto e = FavoriteManager::getInstance()->getFavoriteHubEntry(fromUrl);
				auto url = client->getHubUrl();
				if(e != NULL && !FavoriteManager::getInstance()->isFavoriteHub(url)) {
					e->setServer(url);
					if(strnicmp("adc://", url.c_str(), 6) == 0 || strnicmp("adcs://", url.c_str(), 7) == 0) {
						e->setEncoding(Text::utf8);
						client->setEncoding(const_cast<string*>(&Text::utf8));
					}
					FavoriteManager::getInstance()->updateFavorite(e);
					speak(ADD_STATUS_LINE, STRING(HUB_PERMANENT_REDIRECT_UPDATED));
				} else speak(ADD_STATUS_LINE, STRING(HUB_PERMANENT_REDIRECT));
				fromUrl = Util::emptyString;
			}
			...
			if(!msg.from.getUser() || (!FavoriteManager::getInstance()->isIgnoredUser(msg.from.getUser()))) {
			...
			if(AllUsers != ShownUsers) {
				text[0] = Util::toStringT(ShownUsers) + _T("/") + Util::toStringT(AllUsers) + _T(" ") + TSTRING(HUB_USERS);
			} else {
				text[0] = Util::toStringT(AllUsers) + _T(" ") + TSTRING(HUB_USERS);
			}
			
			int64_t available = client->getAvailable();
			text[1] = Util::formatBytesT(available);
			
			if(AllUsers > 0)
				text[2] = Util::formatBytesT(available / AllUsers) + _T("/") + TSTRING(USER);

			bool update = false;
			for(int i = 0; i < 3; i++) {
				int size = WinUtil::getTextWidth(text[i], ctrlStatus.m_hWnd) + 23;
				if(size != statusSizes[i + 1]) {
					statusSizes[i + 1] = size;
					update = true;
				}
				ctrlStatus.SetText(i + 2, text[i].c_str());
				ctrlStatus.SetIcon(i + 2, hndStatusIcons[StatusIcons::ONLINE_USERS + i]);
			...
			if(!pm.from.getUser() || (!FavoriteManager::getInstance()->isIgnoredUser(pm.from.getUser()))) {
				bool myPM = pm.replyTo == ClientManager::getInstance()->getMe();
				const UserPtr& user = myPM ? pm.to : pm.replyTo;
				if(pm.hub) {
					if(BOOLSETTING(IGNORE_HUB_PMS) && (Wildcard::match(ClientManager::getInstance()->getNicks(user->getCID(), client->getHubUrl())[0], client->getHubChats(), ';', false) == false)) {
					...
					if(BOOLSETTING(IGNORE_BOT_PMS) && (Wildcard::match(ClientManager::getInstance()->getNicks(user->getCID(), client->getHubUrl())[0], client->getHubChats(), ';', false) == false)) {
		...
		} else if(i->first == CHEATING_USER || i->first == GREEN_USER || i->first == YELLOW_USER) {
			CHARFORMAT2 cf;
			...
			switch(i->first) {
				case GREEN_USER:
					cf.crTextColor = SETTING(COLOUR_DUPE);
					break;
				case YELLOW_USER:
					cf.crTextColor = SETTING(SEARCH_ALTERNATE_COLOUR);
					break;
				default:
					cf.crTextColor = SETTING(BAD_CLIENT_COLOUR);
			}

			tstring msg = Text::toT(static_cast<StatusTask&>(*i->second).str);
			if(BOOLSETTING(POPUP_CHEATING_USER) && i->first == CHEATING_USER && msg.length() < 256) {
				WinUtil::showPopup(msg, TSTRING(CHEATING_USER));
			}
...
LRESULT HubFrame::onClose(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& bHandled) {
	if(!closed) {
		...
		if(fhe != NULL && !BOOLSETTING(GLOBAL_HUBFRAME_CONF)) {
...
void HubFrame::addLine(const Identity& i, const tstring& aLine, CHARFORMAT2& cf, bool bUseEmo/* = true*/) {
	string extra;
	if(client->isOp() && (BOOLSETTING(IP_IN_CHAT) || BOOLSETTING(COUNTRY_IN_CHAT))) {
		extra = WinUtil::getIpCountryForChat(i.getIp(), timeStamps);
	}

	if(client->getLogChat()) {
		StringMap params;
	...
	if(timeStamps) {
		ctrlClient.AppendText(i, Text::toT(client->getCurrentNick()), Text::toT("[" + Util::getShortTimeString() + extra + "] "), aLine, cf, bUseEmo);
	} else {
		ctrlClient.AppendText(i, Text::toT(client->getCurrentNick()), !extra.empty() ? Text::toT("[" + extra + "] ") : Util::emptyStringT,  aLine, cf, bUseEmo);
	}
...
void HubFrame::runUserCommand(::UserCommand& uc) {
	...
	string opChat = client->getOpChat();
	if(opChat.find("*") == string::npos && opChat.find("?") == string::npos)
		ucParams["opchat"] = opChat;
...
void HubFrame::onTab() {
	...
	if( (focus == ctrlMessage.m_hWnd) && !WinUtil::isShift() ) 
	{
		...
		while(firstPass || (!firstPass && i < start)) {
			...
			bool found = (strnicmp(nick.c_str(), complete.c_str(), complete.length()) == 0);
...
LRESULT HubFrame::onGetToolTip(int /*idCtrl*/, LPNMHDR pnmh, BOOL& /*bHandled*/) {
	...
	p.x = LOWORD(dwPos);
	p.y = HIWORD(dwPos);
	ctrlStatus.ScreenToClient(&p);

	

	USES_CONVERSION;
	dcpp::ResourceManager *rm = dcpp::ResourceManager::getInstance();
	if (rm!=NULL) {
		//iterate over status bar parts
		for(int idx=1;idx<=4;++idx)
		{
			ctrlStatus.GetRect(idx, &rc);
			if(p.x>=rc.left && p.x<=rc.right && p.y>=rc.top && p.y<=rc.bottom)
			{				
				
				if(idx==1 && client->isSecure())
				{	
					// Connections between this hub are secure	
					wsprintf(nm->lpszText, dcpp::ResourceManager::getInstance()->getStringW(dcpp::ResourceManager::FRAMEHUB_STATUS_HUBSECURE).c_str(), A2W(client->getCipherName().c_str()));			
					return 0;
				} 
				
				
				wcscpy(nm->lpszText, rm->getStringW((dcpp::ResourceManager::Strings)(dcpp::ResourceManager::FRAMEHUB_STATUS_HUBNOTSECURE + idx - 1)).c_str());
...
void HubFrame::closeWindow(const tstring& server) {
...
void HubFrame::on(Redirect, const Client* client, const string& line, bool permanent) noexcept { 
	...
	if(permanent || BOOLSETTING(AUTO_FOLLOW)) {
		// @todo Permanent redirects ignoring auto follow should be considered an implementation detail and be in effect as long as the transition period to ADC as the default protocol is in effect.
		if(permanent) fromUrl = client->getHubUrl();
...
void HubFrame::on(Failed, const Client*, const string& line) noexcept { 
	// Don't clear fromUrl incase first connect fails or times out
...
void HubFrame::on(CheatMessage, const Client*, const string& line, uint32_t aFlag) noexcept {
	...
	if(BOOLSETTING(DISPLAY_CHEATS_IN_MAIN_CHAT)) {
		if(aFlag == DetectionEntry::GREEN) {
			speak(GREEN_USER, line);
		} else if(aFlag == DetectionEntry::YELLOW) {
			speak(YELLOW_USER, line);
		} else {
...
void HubFrame::on(ClientLine, const Client*, const string& line, int type) noexcept {
	switch(type) {
		case MSG_CLIENT:
			addLine(Text::toT(line), WinUtil::m_ChatTextGeneral);
			break;
		case MSG_STATUS:
			addStatus(Text::toT(line), WinUtil::m_ChatTextSystem, false);
			break;
		case MSG_SYSTEM:
			addStatus(Text::toT(line), WinUtil::m_ChatTextSystem);
			break;
	}
}
...
LRESULT HubFrame::onOpenUserLog(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/) {	
	...
	tstring file = Text::toT(Util::validateFileName(LogManager::getInstance()->getPath(LogManager::PM, params)));
...
LRESULT HubFrame::onOpenHubLog(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/) {
	...
	params["hubURL"] = client->getHubUrl();
	params["myNI"] = client->getMyNick(); 
	tstring filename = Text::toT(Util::validateFileName(LogManager::getInstance()->getPath(LogManager::CHAT, params)));
...
LRESULT HubFrame::onCustomDraw(int /*idCtrl*/, LPNMHDR pnmh, BOOL& /*bHandled*/) {
	...
	case CDDS_ITEMPREPAINT: {
			...
			if (client->isOp()) {
				if(ui->getIdentity().isCheckStatus(Identity::BAD_CLIENT)) {
					cd->clrText = SETTING(BAD_CLIENT_COLOUR);
				} else if(ui->getIdentity().isCheckStatus(Identity::BAD_LIST)) {
					cd->clrText = SETTING(BAD_FILELIST_COLOUR);
				} else if(BOOLSETTING(SHOW_SHARE_CHECKED_USERS) && ui->getIdentity().isCheckStatus(Identity::CHECKED)) {
					cd->clrText = SETTING(FULL_CHECKED_COLOUR);
				} else if(Wildcard::match(ui->getIdentity().getNick(), client->getProtectedPrefixes(), ';', false)) {
					cd->clrText = SETTING(PROTECTED_COLOUR);
				}
			}
		...
		if(BOOLSETTING(GET_USER_COUNTRY) && (ctrlUsers.findColumn(cd->iSubItem) == OnlineUser::COLUMN_IP)) {
				...
				|| (WinUtil::getOsMajor() >= 6)) //Vista & Win7
			{
				...
				if(ctrlUsers.GetItemState((int)cd->nmcd.dwItemSpec, LVIS_SELECTED) & LVIS_SELECTED) {
					...
					color = ((BOOLSETTING(USE_CUSTOM_LIST_BACKGROUND) && cd->nmcd.dwItemSpec % 2 != 0) ? HLS_TRANSFORM(WinUtil::bgColor, -9, 0) : WinUtil::bgColor);
					SetBkColor(cd->nmcd.hdc, color);
					SetTextColor(cd->nmcd.hdc, cd->clrText);
				}
