...
private:
	struct Context : Flags {
		...
		Context(Parser& parser, HWND hWnd);
...
tstring HtmlToRtf::convert(const string& html, HWND hWnd) {
	Parser parser(hWnd);
...
void Parser::data(const string& data) {
	ret += ChatCtrl::rtfEscape(Text::toT(Text::toDOS(data)));
}
...
Parser::Context::Context(Parser& parser, HWND hWnd) {
	// create a default context with the Rich Edit control's current formatting.
	auto hdc = ::GetDC(hWnd);
	LOGFONT lf;
	WinUtil::decodeFont(Text::toT(SETTING(TEXT_FONT)), lf);

	font = parser.addFont("\\fnil\\fcharset" + Util::toString(lf.lfCharSet) + " " + Text::fromT(lf.lfFaceName));
	fontSize = rtfFontSize(static_cast<float>(abs(lf.lfHeight)) / (static_cast<float>(::GetDeviceCaps(hdc, LOGPIXELSX) / 96.0)));
	if(lf.lfWeight >= FW_BOLD) { setFlag(Bold); }
	if(lf.lfItalic) { setFlag(Italic); }

	textColor = parser.addColor(SETTING(TEXT_GENERAL_FORE_COLOR));
	bgColor = parser.addColor(SETTING(TEXT_GENERAL_BACK_COLOR));
	::ReleaseDC(hWnd, hdc);
}
