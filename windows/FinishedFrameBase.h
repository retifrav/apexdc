...
public:
	...
	LRESULT onContextMenu(UINT /*uMsg*/, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
		if (reinterpret_cast<HWND>(wParam) == ctrlList && ctrlList.GetSelectedCount() > 0) { 
			...
			if(BOOLSETTING(SHOW_SHELL_MENU) && (ctrlList.GetSelectedCount() == 1) && (WinUtil::getOsMajor() >= 5)) {
...
protected:
	...
	void updateStatus() {
		...
		ctrlStatus.SetText(1, (Util::toStringT(count) + _T(" ") + TSTRING(ITEMS)).c_str());
		ctrlStatus.SetText(2, Util::formatBytesT(totalBytes).c_str());
		ctrlStatus.SetText(3, (Util::formatBytesT(count > 0 ? totalSpeed / count : 0) + _T("/s")).c_str());
	}
