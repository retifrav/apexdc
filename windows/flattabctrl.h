...
public:
	...
	void addTab(HWND hWnd, COLORREF color = RGB(0, 0, 0), long icon = 0, long stateIcon = 0, bool mini = false) {
		TabInfo* i = new TabInfo(hWnd, color, icon, (stateIcon != 0) ? stateIcon : icon);
		dcassert(getTabInfo(hWnd) == NULL);
		i->mini = mini;
		if((icon == IDR_HUB || icon == IDR_PRIVATE) || !BOOLSETTING(NON_HUBS_FRONT)) {
			tabs.push_back(i);
			if(SETTING(TABS_POS) == 0 || SETTING(TABS_POS) == 1 || BOOLSETTING(NON_HUBS_FRONT))
	...
	void setColor(HWND aWnd, COLORREF color) {
		...
		if(ti != NULL) {
			ti->pen.DeleteObject();
			ti->pen.CreatePen(PS_SOLID, 1, (BOOLSETTING(ZION_TABS) ? GetSysColor(COLOR_BTNSHADOW) : color));
			ti->color = color;
			Invalidate();
		}
	}
	...
	void changeTabsStyle() {
		for(TabInfo::List::iterator i = tabs.begin(); i != tabs.end(); ++i) {
			(*i)->pen.DeleteObject();
			(*i)->pen.CreatePen(PS_SOLID, 1, (BOOLSETTING(ZION_TABS) ? GetSysColor(COLOR_BTNSHADOW) : (*i)->color));
		}
		Invalidate();
	}
	...
	LRESULT onPaint(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/) {
		...
		if(GetUpdateRect(&rc, FALSE)) {
			...
			HPEN oldpen = (!BOOLSETTING(ZION_TABS)) ? dc.SelectPen(black) : dc.SelectPen(facepen);
			for(int r = 0; r < rows; r++) {
				dc.MoveTo(rc.left, r*getTabHeight());
				dc.LineTo(rc.right, r*getTabHeight());
				if(BOOLSETTING(ZION_TABS))
...
private:
	...
	public:
		...
		TabInfo(HWND aWnd, COLORREF c, long icon, long stateIcon) : hWnd(aWnd), color(c), tabIcon(icon), len(0), xpos(0), row(0), dirty(false), hIcon(NULL), hStateIcon(NULL), bState(false) { 
			if (icon != 0)
			...
			pen.CreatePen(PS_SOLID, 1, (!BOOLSETTING(ZION_TABS)) ? c : GetSysColor(COLOR_BTNSHADOW));
		...
		int getWidth() const {
			return (!mini ? (dirty ? boldSize.cx : size.cx) : 0) + FT_EXTRA_SPACE + (hIcon != NULL ? 10 : 0);
		}
	};
