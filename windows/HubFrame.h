...
public:
	...
	static bool openWindow(const tstring& server, int chatusersplit = 0, bool userliststate = true);
	static void resortUsers();	
	static void closeDisconnected();
	static void reconnectDisconnected();
	static void closeAll(size_t thershold = 0);
	static void closeWindow(const tstring& server);
	...
	LRESULT onIgnore(UINT /*uMsg*/, WPARAM /*wParam*/, HWND /*lParam*/, BOOL& /*bHandled*/) {
		...
		if(client->isConnected()) {
			while( (i = ctrlUsers.GetNextItem(i, LVNI_SELECTED)) != -1) {
				FavoriteManager::getInstance()->addIgnoredUser(((OnlineUser*)ctrlUsers.getItemData(i))->getUser());
			}
		}
		return 0;
	}
	...
	LRESULT onUnignore(UINT /*uMsg*/, WPARAM /*wParam*/, HWND /*lParam*/, BOOL& /*bHandled*/) {
		...
		if(client->isConnected()) {
			while( (i = ctrlUsers.GetNextItem(i, LVNI_SELECTED)) != -1) {
				FavoriteManager::getInstance()->removeIgnoredUser(((OnlineUser*)ctrlUsers.getItemData(i))->getUser());
			}
		}
		return 0;
	}
...
private:
	enum Tasks { UPDATE_USER_JOIN, UPDATE_USER, REMOVE_USER, ADD_CHAT_LINE,
		...
		GET_SHUTDOWN, SET_SHUTDOWN, GREEN_USER, YELLOW_USER
	};
		...
		ctrlFilterSelContainer(WC_COMBOBOX, this, FILTER_MESSAGE_MAP)
	{
		...
		if(FavoriteManager::getInstance()->getFavoriteHubEntry(Text::fromT(server)) != NULL && !BOOLSETTING(GLOBAL_HUBFRAME_CONF)) {
	...
	~HubFrame() {
		...
		for(int idx=0;idx<5;++idx)
			DestroyIcon(hndStatusIcons[idx]);
	...
	tstring redirect;
	string fromUrl;								// Hub address originally connected to, used during permanent redirects
	bool timeStamps;
	bool showJoins;
	bool favShowJoins;
	tstring complete;
	...
	HICON hndStatusIcons[5];
	CMultiPaneStatusBarCtrl ctrlStatus;
	...
	void on(Redirect, const Client*, const string&, bool) noexcept;
	...
	void on(CheatMessage, const Client*, const string&, uint32_t) noexcept;	
	void on(HubTopic, const Client*, const string&) noexcept;
	void on(UserReport, const Client*, const Identity&) noexcept;
	void on(ClientLine, const Client*, const string&, int) noexcept;
