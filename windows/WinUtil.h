...
static const toolbarButton ToolbarButtons[] = {
	...
	{IDC_LIMITER, 19, true, ResourceManager::ENABLE_LIMITING},
	...
	{ID_FILE_QUICK_CONNECT, 24, false, ResourceManager::MENU_QUICK_CONNECT}
};
...
public:
	...
	static string getMagnet(const TTHValue& /*aHash*/, const string& /*aFile*/, int64_t);
	...
	static string formatTime(time_t rest);
	...
	static void MDIActivate(HWND hwndChild);
	...
	static void createShortcut(const string& aPath, const string& aLinkName, const string& aWorkingDir, const string& aDesc, int nFolder);
	static void moveDirectory(const string& aSource, const string& aTarget, bool bSilent = true);
	static void removeDirectory(const string& aSource);
	static void alterWinFirewall(bool bState);
	static void createAddRemoveEntry();
...
// FDM extension
	...
	static string getIpCountryForChat(string ip, bool ts) {
		if(!ip.empty() && Util::fileExists(Util::getPath(Util::PATH_RESOURCES) + "GeoIPCountryWhois.csv") && BOOLSETTING(COUNTRY_IN_CHAT) && BOOLSETTING(IP_IN_CHAT)) {
			return ts ? " | " + ip + " | " + Util::getIpCountry(ip) : " " + ip + " | " + Util::getIpCountry(ip);
		} else if (!ip.empty() && Util::fileExists(Util::getPath(Util::PATH_RESOURCES) + "GeoIPCountryWhois.csv") && BOOLSETTING(COUNTRY_IN_CHAT)) {
...
// FDM extension
	...
	static tstring getCompileDate(tstring sFormat = _T("%Y-%m-%d %H:%M"));
