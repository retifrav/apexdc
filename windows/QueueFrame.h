...
{
public:
	DECLARE_FRAME_WND_CLASS_EX(_T("QueueFrame"), IDR_QUEUE, CS_DBLCLKS, COLOR_3DFACE);
...
private:
	...
	struct UpdateTask : FastAlloc<UpdateTask>, public Task {
		UpdateTask(const QueueItem& source) : target(source.getTarget()), size(source.getSize()) { }
		string target;
		int64_t size;
	};
