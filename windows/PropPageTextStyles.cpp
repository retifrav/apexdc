...
PropPageTextStyles::clrs PropPageTextStyles::colours[] = {
	...
	{ResourceManager::SETTINGS_DUPE_COLOR,	SettingsManager::COLOUR_DUPE, 0},
...
LRESULT PropPageTextStyles::onEditForeColor(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
	...
	if(iNdx == TS_GENERAL) fg = TextStyles[ iNdx ].crTextColor;
	RefreshPreview();
	return TRUE;
}
...
void PropPageTextStyles::RefreshPreview() {
	...
	for ( int i = 0; i < TS_LAST; i++ ) {
		m_Preview.AppendText(id, _T("My nick"), _T("12:34 "), Text::toT(TextStyles[i].m_sPreviewText), TextStyles[i], false);
	}
...
LRESULT PropPageTextStyles::onDefaultStyles(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/) {
	...
	TextStyles[ TS_GENERAL ].crBackColor = bg;
	TextStyles[ TS_GENERAL ].crTextColor = fg;
	TextStyles[ TS_GENERAL ].dwEffects = 0;

	TextStyles[ TS_MYNICK ].crBackColor = bg;
	TextStyles[ TS_MYNICK ].crTextColor = fg;
	TextStyles[ TS_MYNICK ].dwEffects = CFE_BOLD;

	TextStyles[ TS_MYMSG ].crBackColor = bg;
	TextStyles[ TS_MYMSG ].crTextColor = fg;
	TextStyles[ TS_MYMSG ].dwEffects = 0;

	TextStyles[ TS_PRIVATE ].crBackColor = bg;
	TextStyles[ TS_PRIVATE ].crTextColor = fg;
	TextStyles[ TS_PRIVATE ].dwEffects = CFE_ITALIC;

	TextStyles[ TS_SYSTEM ].crBackColor = bg;
	TextStyles[ TS_SYSTEM ].crTextColor = RGB(0, 128, 64);
	TextStyles[ TS_SYSTEM ].dwEffects = CFE_BOLD;

	TextStyles[ TS_SERVER ].crBackColor = bg;
	TextStyles[ TS_SERVER ].crTextColor = RGB(0, 128, 64);
	TextStyles[ TS_SERVER ].dwEffects = CFE_BOLD;

	TextStyles[ TS_TIMESTAMP ].crBackColor = bg;
	TextStyles[ TS_TIMESTAMP ].crTextColor = fg;
	TextStyles[ TS_TIMESTAMP ].dwEffects = 0;

	TextStyles[ TS_URL ].crBackColor = bg;
	TextStyles[ TS_URL ].crTextColor = RGB(0,0,255);
	TextStyles[ TS_URL ].dwEffects = CFE_LINK;

	TextStyles[ TS_FAVORITE ].crBackColor = bg;
	TextStyles[ TS_FAVORITE ].crTextColor = fg;
	TextStyles[ TS_FAVORITE ].dwEffects = CFE_BOLD;

	TextStyles[ TS_OP ].crBackColor = bg;
...
LRESULT PropPageTextStyles::onImport(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/) {
	...
	if(WinUtil::browseFile(x, m_hWnd, false, x, types, defExt) == IDOK) {
		...
		if(xml.findChild(("Settings"))) {
			...
			importData("ColourDupe", COLOUR_DUPE);
			importData("UseCustomListBackground", USE_CUSTOM_LIST_BACKGROUND);
			importData("ZionTabs", ZION_TABS);
		}
		xml.resetCurrentChild();
		xml.stepOut();
	}
...
LRESULT PropPageTextStyles::onExport(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/) {
	...
	exportData("ColourDupe", COLOUR_DUPE);
	exportData("UseCustomListBackground", USE_CUSTOM_LIST_BACKGROUND);
	exportData("ZionTabs", ZION_TABS);
