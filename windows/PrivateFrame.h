...
private:
	...
	string getCustomAway(StringMap& params) {
		string defAway = Util::getAwayMessage(params);
		if(ctrlClient.getClient() == NULL)
			return defAway; // might be custom away but not from favs
		return FavoriteManager::getInstance()->getAwayMessage(ctrlClient.getClient()->getHubUrl(), params);
	}

	void setClient(Client* aClient = NULL) {
		Client* client = ((aClient != NULL) ? aClient : ClientManager::getInstance()->getUserClient(replyTo));
		ctrlClient.setClient(client);
		if(client) hubname = Text::toT(client->getHubName());
	}
	...
	void on(ClientManagerListener::ClientDisconnected, const Client* c) noexcept {
		if(ctrlClient.getClient() == c) {
