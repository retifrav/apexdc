...
void DirectoryListingFrame::updateStatus() {
	if(!searching && !updating && ctrlStatus.IsWindow()) {
		...
		tstring tmp = TSTRING(ITEMS) + _T(": ") + Util::toStringT(cnt);
		bool u = false;
...
LRESULT DirectoryListingFrame::onDoubleClickFiles(int /*idCtrl*/, LPNMHDR pnmh, BOOL& /*bHandled*/) {
	...
	if(t != NULL && item->iItem != -1) {
		...
		if(ii->type == ItemInfo::FILE) {
			try {
				string dir = FavoriteManager::getInstance()->getDownloadDirectory(Util::getFileExt(ii->file->getName()));
				dl->download(ii->file, dir + Text::fromT(ii->getText(COLUMN_FILENAME)), false, WinUtil::isShift() ? QueueItem::HIGH : QueueItem::DEFAULT);
...
LRESULT DirectoryListingFrame::onDownloadDir(WORD , WORD , HWND , BOOL& ) {
	...
	if(t != NULL) {
		...
		try {
			dl->download(dir, SETTING(DOWNLOAD_DIRECTORY), WinUtil::isShift() ? QueueItem::HIGH : QueueItem::DEFAULT);
		} catch(const Exception& e) {
			ctrlStatus.SetText(STATUS_TEXT, Text::toT(e.getError()).c_str());
		}
	}
...
LRESULT DirectoryListingFrame::onDownloadDirWithPrio(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/) {
	...
	if(t != NULL) {
		...
		try {
			dl->download(dir, SETTING(DOWNLOAD_DIRECTORY), WinUtil::isShift() ? QueueItem::HIGH : p);
		} catch(const Exception& e) {
			ctrlStatus.SetText(STATUS_TEXT, Text::toT(e.getError()).c_str());
		}
	}
...
LRESULT DirectoryListingFrame::onDownloadDirTo(WORD , WORD , HWND , BOOL& ) {
	...
	if(t != NULL) {
		...
		if(WinUtil::browseDirectory(target, m_hWnd)) {
			...
			try {
				dl->download(dir, Text::fromT(target), WinUtil::isShift() ? QueueItem::HIGH : QueueItem::DEFAULT);
			} catch(const Exception& e) {
				ctrlStatus.SetText(STATUS_TEXT, Text::toT(e.getError()).c_str());
			}
		}
	...
	while( (i = ctrlList.GetNextItem(i, LVNI_SELECTED)) != -1) {
		...
		try {
			if(ii->type == ItemInfo::FILE) {
				...
				dl->download(ii->file, target + Text::fromT(ii->getText(COLUMN_FILENAME)), view, (WinUtil::isShift() || view) ? QueueItem::HIGH : prio);
			} else if(!view) {
				dl->download(ii->dir, target, WinUtil::isShift() ? QueueItem::HIGH : prio);
			} 
		} catch(const Exception& e) {
			ctrlStatus.SetText(STATUS_TEXT, Text::toT(e.getError()).c_str());
		}
...
void DirectoryListingFrame::downloadList(bool view /* = false */, QueueItem::Priority prio /* = QueueItem::Priority::DEFAULT */) {
	int i=-1;
	while( (i = ctrlList.GetNextItem(i, LVNI_SELECTED)) != -1) {
		ItemInfo* ii = ctrlList.getItemData(i);

		string target = FavoriteManager::getInstance()->getDownloadDirectory(Util::getFileExt(Text::fromT(ii->getText(COLUMN_FILENAME))));
		...
		try {
			if(ii->type == ItemInfo::FILE) {
				...
				if(WinUtil::browseFile(target, m_hWnd)) {
					...
					dl->download(ii->file, Text::fromT(target), false, WinUtil::isShift() ? QueueItem::HIGH : QueueItem::DEFAULT);
				}
			} else {
				tstring target = Text::toT(SETTING(DOWNLOAD_DIRECTORY));
				if(WinUtil::browseDirectory(target, m_hWnd)) {
					WinUtil::addLastDir(target);
					dl->download(ii->dir, Text::fromT(target), WinUtil::isShift() ? QueueItem::HIGH : QueueItem::DEFAULT);
				}
			} 
		} catch(const Exception& e) {
			ctrlStatus.SetText(STATUS_TEXT, Text::toT(e.getError()).c_str());
...
LRESULT DirectoryListingFrame::onMatchQueue(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/) {
	...
	ctrlStatus.SetText(STATUS_TEXT, CTSTRING_F(MATCHED_FILES, x));
	...
	if(ctrlList.GetSelectedCount() == 1) {
		...
		if(ii->type == ItemInfo::FILE) {
			if(newId < (int)targets.size()) {
				try {
					dl->download(ii->file, targets[newId], false, WinUtil::isShift() ? QueueItem::HIGH : QueueItem::DEFAULT);
				} catch(const Exception& e) {
					ctrlStatus.SetText(STATUS_TEXT, Text::toT(e.getError()).c_str());
				} 
			} else {
...
LRESULT DirectoryListingFrame::onDownloadTargetDir(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/) {
	...
	if(t != NULL) {
		...
		try {
			dcassert(newId < (int)WinUtil::lastDirs.size());
			dl->download(dir, Text::fromT(WinUtil::lastDirs[newId]), WinUtil::isShift() ? QueueItem::HIGH : QueueItem::DEFAULT);
		} catch(const Exception& e) {
			ctrlStatus.SetText(STATUS_TEXT, Text::toT(e.getError()).c_str());
		}
	}
...
LRESULT DirectoryListingFrame::onDownloadWholeFavoriteDirs(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/) {
	...
	if(t != NULL) {
		...
		try {
			FavoriteManager::FavDirList spl = FavoriteManager::getInstance()->getFavoriteDirs();
			dcassert(newId < (int)spl.size());
			dl->download(dir, spl[newId].dir, WinUtil::isShift() ? QueueItem::HIGH : QueueItem::DEFAULT);
		} catch(const Exception& e) {
			ctrlStatus.SetText(STATUS_TEXT, Text::toT(e.getError()).c_str());
		}
	}
...
void DirectoryListingFrame::runUserCommand(UserCommand& uc) {
	...
	while((sel = ctrlList.GetNextItem(sel, LVNI_SELECTED)) != -1) {
		...
		if(ii->type == ItemInfo::FILE) {
			...
			ucParams["fileMN"] = WinUtil::getMagnet(ii->file->getTTH(), ii->file->getName(), ii->file->getSize());
		} else {
...
LRESULT DirectoryListingFrame::onCopy(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/) {
	...
	while( (i = ctrlList.GetNextItem(i, LVNI_SELECTED)) != -1) {
		...
		switch (wID) {
			...
			case IDC_COPY_LINK:
				if(ii->type == ItemInfo::FILE)
					sCopy = Text::toT(WinUtil::getMagnet(ii->file->getTTH(), ii->file->getName(), ii->file->getSize()));
				break;
			case IDC_COPY_TTH:
...
LRESULT DirectoryListingFrame::onCustomDraw(int /*idCtrl*/, LPNMHDR pnmh, BOOL& /*bHandled*/) {
	...
	case CDDS_ITEMPREPAINT: {
		...
		if(ii != NULL && dl->getUser() != ClientManager::getInstance()->getMe()) {
			if(ii->type == ItemInfo::FILE) {
				StringList targets = QueueManager::getInstance()->getTargets(ii->file->getTTH());
				if(targets.size() > 0) {
					cd->clrText = SETTING(SEARCH_ALTERNATE_COLOUR);	
				} else if(ii->file->getSize() > 0 && ShareManager::getInstance()->isTTHShared(ii->file->getTTH())) {
