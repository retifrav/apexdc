	...
	if(i == frames.end()) {
		...
		if(Util::getAway()) {
			if(!(BOOLSETTING(NO_AWAYMSG_TO_BOTS) && user->isSet(User::BOT))) {
				...
				p->sendMessage(Text::toT(p->getCustomAway(params)));
			}
		}
		...
		if(BOOLSETTING(POPUP_NEW_PM)) {
			if(BOOLSETTING(PM_PREVIEW)) {
				...
				WinUtil::showPopup(message, TSTRING(PRIVATE_MESSAGE));
			} else {
...
void PrivateFrame::onEnter()
{
	...
	if(ctrlMessage.GetWindowTextLength() > 0) {
		...
		if(s[0] == '/') {
			...
			if(PluginManager::getInstance()->onChatCommandPM(replyTo, Text::fromT(s))) {
				// Plugins, chat commands
				resetText = true;
			} else if(WinUtil::checkCommand(s, param, message, status, thirdPerson)) {
				if(!message.empty()) {
			...
			} else if((stricmp(s.c_str(), _T("clear")) == 0) || (stricmp(s.c_str(), _T("cls")) == 0) || (stricmp(s.c_str(), _T("c")) == 0)) {
			...
			} else if(stricmp(s.c_str(), _T("pubstats")) == 0) {
				sendMessage(Text::toT(WinUtil::generateStats()));
			} else if(stricmp(s.c_str(), _T("help")) == 0 || stricmp(s.c_str(), _T("h")) == 0) {
...
void PrivateFrame::addLine(const Identity& from, const tstring& aLine, CHARFORMAT2& cf) {
	...
	string extra;
	if((ctrlClient.getClient() != NULL) && (BOOLSETTING(IP_IN_CHAT) || BOOLSETTING(COUNTRY_IN_CHAT))) {
		if(ctrlClient.getClient()->isOp())
	...
	const tstring& currentNick = Text::toT(ctrlClient.getClient() ? ctrlClient.getClient()->getCurrentNick() : SETTING(NICK));
	if(BOOLSETTING(TIME_STAMPS)) {
		ctrlClient.AppendText(from, currentNick, Text::toT("[" + Util::getShortTimeString() + extra + "] "), aLine, cf);
	} else {
		ctrlClient.AppendText(from, currentNick, !extra.empty() ? Text::toT("[" + extra + "] ") : Util::emptyStringT, aLine, cf);
	}
...
void PrivateFrame::updateTitle() {
	...
	if(hubs.second) {	
		...
		tstring status = _T(" *** ") + TSTRING(USER_WENT_OFFLINE) + _T(" [") + WinUtil::getNicks(replyTo) + _T(" - ") + hubname + _T("] ***");
		if(BOOLSETTING(STATUS_IN_CHAT)) {
			addLine(status, WinUtil::m_ChatTextServer);
		} else {
			addClientLine(status);
...
void PrivateFrame::readLog() {
	if (SETTING(PM_LAST_LINES_LOG) == 0) return;
	...
	try {
		...
		int linesLog = SETTING(PM_LAST_LINES_LOG);

		int i = linesCount > (linesLog + 1) ? linesCount - linesLog : 0;

		for(; i < linesCount; ++i){
			ctrlClient.AppendText(Identity(NULL, 0), _T("- "), _T(""), Text::toT(lines[i]), WinUtil::m_ChatTextLog, true);
		}
