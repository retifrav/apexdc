...
const tstring QueueFrame::QueueItemInfo::getText(int col) const {
	switch(col) {
		...
		case COLUMN_SEGMENTS: {
			tstring textSegments = _T("0/0");
			QueueManager::getInstance()->lockedOperation([this, &textSegments](const QueueItem::StringMap& queue) {
				auto qi = queue.find(const_cast<string*>(&getTarget()));
				if(qi != queue.end())
					textSegments = Util::toStringT(qi->second->getDownloads().size()) + _T("/") + Util::toStringT(qi->second->getMaxSegments());
			});
			return textSegments;
		}
		case COLUMN_SIZE: return (getSize() == -1) ? TSTRING(UNKNOWN) : Util::formatBytesT(getSize());
		case COLUMN_DOWNLOADED: {
			int64_t downloadedBytes = getDownloadedBytes();
			return (getSize() > 0) ? Util::formatBytesT(downloadedBytes) + _T(" (") + Util::toStringT((double)downloadedBytes*100.0/(double)getSize()) + _T("%)") : Util::emptyStringT;
		}
		...
		case COLUMN_EXACT_SIZE: return (getSize() == -1) ? TSTRING(UNKNOWN) : Text::toT(Util::formatExactSize(getSize()));
		case COLUMN_ERRORS: {
			tstring tmp;
...
void QueueFrame::addQueueList(const QueueItem::StringMap& li) {
	...
	for(auto j = li.cbegin(); j != li.cend(); ++j) {
		QueueItemInfo* ii = new QueueItemInfo(j->second);
...
void QueueFrame::on(QueueManagerListener::Removed, const QueueItem* aQI) noexcept {
	speak(REMOVE_ITEM, new StringTask(aQI->getTarget()));
}

void QueueFrame::on(QueueManagerListener::Moved, const QueueItem*, const string& oldTarget) noexcept {
...
void QueueFrame::on(QueueManagerListener::SourcesUpdated, const QueueItem* aQI) noexcept {
...
void QueueFrame::removeSelected() {
	...
	if(checkState == BST_CHECKED || ::MessageBox(m_hWnd, CTSTRING(REALLY_REMOVE), _T(APPNAME) _T(" ") _T(VERSIONSTRING), CTSTRING(DONT_ASK_AGAIN), MB_YESNO | MB_ICONQUESTION | MB_DEFBUTTON2, checkState) == IDYES)
...
LRESULT QueueFrame::onContextMenu(UINT /*uMsg*/, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
	...
	if (reinterpret_cast<HWND>(wParam) == ctrlQueue && ctrlQueue.GetSelectedCount() > 0) { 
		...
		for(int i = IDC_SEGMENTONE; i <= IDC_SEGMENTTEN; i++)
			segmentsMenu.AppendMenu(MF_STRING, i, (Util::toStringT(i - 109) + _T(" ") + TSTRING(SEGMENTS)).c_str());
...
LRESULT QueueFrame::onSegments(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/) {
	...
	while( (i = ctrlQueue.GetNextItem(i, LVNI_SELECTED)) != -1) {
		...
		QueueManager::getInstance()->lockedOperation([ii, wID](const QueueItem::StringMap& queue) {
			auto qi = queue.find(const_cast<string*>(&ii->getTarget()));
			if(qi != queue.end())
				qi->second->setMaxSegments((uint8_t)(wID - 109));
		});
...
LRESULT QueueFrame::onPreviewCommand(WORD /*wNotifyCode*/, WORD wID, HWND /*hWndCtl*/, BOOL& /*bHandled*/) {	
	if(ctrlQueue.GetSelectedCount() == 1) {
		...
		QueueManager::getInstance()->lockedOperation([ii, wID](const QueueItem::StringMap& queue) {
			auto qi = queue.find(const_cast<string*>(&ii->getTarget()));
			if(qi != queue.end())
				WinUtil::RunPreviewCommand(wID - IDC_PREVIEW_APP, qi->second->getTempTarget());
		});
	}
	return 0;
}
...
void QueueFrame::onRechecked(const string& target, const string& message) {
	speak(UPDATE_STATUS, new StringTask(STRING_F(INTEGRITY_CHECK, message.c_str() % target.c_str())));
}
