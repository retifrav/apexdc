	...
	public:
		...
		const tstring getText(uint8_t col) const {
			switch(col) {
				...
				case COLUMN_EXACTSIZE: return Text::toT(type == DIRECTORY ? Util::formatExactSize(dir->getTotalSize()) : Util::formatExactSize(file->getSize()));
				case COLUMN_SIZE: return  type == DIRECTORY ? Util::formatBytesT(dir->getTotalSize()) : Util::formatBytesT(file->getSize());
	...
	int run() {
		try {
			if(!mFile.empty()) {
				...
				mWindow->refreshTree(Text::toT(Util::fromAdcFile(mWindow->dl->updateXML(mTxt))));
			}
