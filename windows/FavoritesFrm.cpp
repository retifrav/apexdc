...
LRESULT FavoriteHubsFrame::onContextMenu(UINT /*uMsg*/, WPARAM wParam, LPARAM lParam, BOOL& bHandled) {
	if(reinterpret_cast<HWND>(wParam) == ctrlHubs) {
		...
		hubsMenu.EnableMenuItem(IDC_MOVE_UP, ::IsWindowEnabled(ctrlUp.m_hWnd) ? status : MFS_DISABLED);
		hubsMenu.EnableMenuItem(IDC_MOVE_DOWN, ::IsWindowEnabled(ctrlDown.m_hWnd) ? status : MFS_DISABLED);
...
LRESULT FavoriteHubsFrame::onRemove(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/) {
	...
	SettingsManager::getInstance()->set(SettingsManager::CONFIRM_HUB_REMOVAL, checkState != BST_CHECKED);
	return 0;
}
