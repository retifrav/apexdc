			...
			if(s->type == Search::TYPE_FILE) // this is response to TYPE_FILE, check sources first
			{
				...
				while(xml.findChild("Source"))
				{
					...
					else
					{
						...
						SearchResultPtr sr(new SearchResult(HintedUser(source->getUser(), DHT::getInstance()->getHubUrl()), SearchResult::TYPE_FILE, 0, 0, size, s->term, DHT::getInstance()->getHubName(), i4, TTHValue(s->term), token));
						if(!source->isOnline())
						{
