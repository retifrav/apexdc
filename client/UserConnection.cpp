...
void UserConnection::on(BufferedSocketListener::Line, const string& aLine) noexcept {
	...
	if(aLine.length() < 2) {
		fire(UserConnectionListener::ProtocolError(), this, STRING(INVALID_DATA));
		return;
	}
	...
	if(aLine[0] == 'C' && !isSet(FLAG_NMDC)) {
		if(!Text::validateUtf8(aLine)) {
			fire(UserConnectionListener::ProtocolError(), this, STRING(NON_UTF8_ADC));
			return;
		}
		dispatch(aLine);
		return;
...
void UserConnection::connect(const string& aServer, uint16_t aPort, uint16_t localPort, BufferedSocket::NatRoles natRole, UserPtr user) {
	dcassert(!socket);
	...
	socket->connect(aServer, aPort, localPort, natRole, isSet(FLAG_SECURE), true, true);
}
...
void UserConnection::accept(const Socket& aServer) {
	...
	socket->accept(aServer, isSet(FLAG_SECURE), true);
}
