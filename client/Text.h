...
namespace Text {
	...
	inline wstring utf8ToWide(const string& str) noexcept {
		wstring tmp;
		BOOST_VERIFY(&utf8ToWide(str, tmp) == &tmp);
		return tmp;
	}
	...
	inline string wideToUtf8(const wstring& str) noexcept {
		string tmp;
		const string& ret = wideToUtf8(str, tmp);
		if (&ret == &tmp)
			return tmp;	// avoid copy, which would have happened with simple   return wideToUtf8(str, tmp);
		return ret;
	}
